...
namespace ServerSideProgrammingAssignment.Service
{
    ...
    public class ImageHelper
    {
        ...
        public static byte[] AddTextToImage(byte[] imageBytes, params (string text, (float x, float y) position, int fontSize, string colorHex, string colorHex2)[] texts)
        {
            ...
            image.Clone(img =>
            {
                foreach (var (text, (x, y), fontSize, colorHex, colorHex2) in texts)
                {
                    var font = SystemFonts.CreateFont("Verdana", fontSize);
                    var color = Rgba32.ParseHex(colorHex);
                    var color2 = Rgba32.ParseHex(colorHex2);

                    var res = img.DrawText(text, font, Brushes.Solid(color), Pens.Solid(color2, 2), new PointF(x, y));
                }
            })
